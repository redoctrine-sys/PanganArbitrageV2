-- ═══════════════════════════════════════
-- 009 — get_sp2kp_latest: include every in-scope city
--
-- Sebelumnya RPC bertumpu pada `prices_raw` sebagai sumber baris, sehingga
-- kota dalam scope (prefix 31–36/51/52) yang BELUM punya data SP2KP —
-- mis. 6 kota DKI Jakarta — tidak pernah muncul di grid By City / By
-- Commodity. Phase 1 ingin grid kota lengkap (133 kota target) walaupun
-- sebagian belum punya harga, supaya admin tahu cakupan inventory dan
-- bisa mengisi koordinat di /dashboard/admin/cities lebih dulu.
--
-- Solusi: ganti basis baris menjadi `scope_cities × commodities`, lalu
-- LEFT JOIN ke ranked prices. Kota tanpa data muncul dengan
-- price_latest / date_latest / het_ha = NULL. Frontend sudah siap untuk
-- nilai null (formatRupiah, calcChangePct, dst.).
--
-- Re-runnable. Mempertahankan SECURITY DEFINER (lihat 008).
-- ═══════════════════════════════════════
...
RETURNS TABLE(
  ...
  -- Cities table sudah pre-filtered: prefix 52 hanya berisi 5 kota Lombok,
  -- prefix lain semuanya in-scope. Cukup filter prefix di sini.
  scope_cities AS (
    SELECT
      c.kode_wilayah,
      c.name AS city_raw,
  ...
  -- Hanya 17 komoditas SP2KP (Phase 1). Phase 2 akan tambah komoditas
  -- non-SP2KP yang TIDAK boleh muncul di tab SP2KP.
